...
This page will teach you how to install and use the afcharts python package to easily create accessible charts in the Analysis Function style.

Check out the rest of the cookbook for more detailed code examples and further guidance.

## Installation

afcharts is available at the Python Package Index (PyPI). Python and `pip` are required for installation (see [Installing Packages (python.org)](https://packaging.python.org/en/latest/tutorials/installing-packages/)). To install afcharts, open a terminal and execute:
```bash
pip install afcharts
```

Alternatively, clone the [Git repository](https://github.com/best-practice-and-impact/afcharts-py) and, from the root directory, install afcharts with `pip` or `uv`:
```bash
# with pip
pip install .
```
...
Use `get_af_colours()` to get a list of colours from any of the [Analysis Function colour palettes](https://analysisfunction.civilservice.gov.uk/policy-store/codes-for-accessible-colours/). For example:
```{python}
#| eval: false
...
See the [available colour palettes](04-colour-palettes.qmd#available-colour-palettes) and more information on using afcharts colours in [Colours](04-colour-palettes.qmd).

Pass the list of colours to your plotting function as demonstrated in [Matplotlib examples](02-matplotlib-usage.qmd), [Plotly examples](03-plotly-usage.qmd) and the examples below.
...
See [Matplotlib examples](02-matplotlib-usage.qmd) for many more examples.
...
for i, (country, data) in enumerate(df.groupby("country")):
    plt.plot(data["year"], data["lifeExp"], label=country, color=categorical[i])
...
fig.update_layout(template="afcharts")
