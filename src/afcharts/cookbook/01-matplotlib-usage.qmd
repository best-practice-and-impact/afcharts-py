...
df = gapminder().query("year in [1967, 2007] & country in ['United Kingdom', 'Ireland', 'France', 'Belgium']")
...
for i, year in enumerate(sorted(df["year"].unique())):
    ...
    plt.bar(x + i * bar_width, df_year["lifeExp"], width=bar_width, label=str(year), color=duo[i % len(duo)])
...
df["lifeExpGrouped"] = pd.cut(df["lifeExp"], bins=[0, 75, float("inf")], labels=["Under 75", "75 and over"])

# Group by continent and life expectancy group
grouped = df.groupby(["continent", "lifeExpGrouped"], observed=True).size().reset_index(name="n_countries")

# Pivot to get proportions
pivot_df = grouped.pivot(index="continent", columns="lifeExpGrouped", values="n_countries").fillna(0)
...
plt.hist(df["lifeExp"], bins=range(int(df["lifeExp"].min()), int(df["lifeExp"].max()) + 5, 5), edgecolor="white")
...
df_grouped = df[df["continent"] != "Oceania"].groupby(["continent", "year"], observed=True)["pop"].sum().reset_index()
...
colours = {country: focus[0 if country == "Brazil" else 1] for country in top5["country"].unique()}
...
fig<div class="chart-title">The U.S.A. is the most populous country in the Americas</div>
<div class="chart-subtitle">Population of countries in the Americas (millions), 2007</div>
