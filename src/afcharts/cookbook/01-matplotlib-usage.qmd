...
# Load gapminder data and filter year 2007
df = px.data.gapminder()
...
ax.set_xlabel("GDP (US$, inflation-adjusted)")
ax.set_ylabel("Life expectancy (years)")

ax.xaxis.set_major_formatter(FuncFormatter(lambda x, pos: f"{int(x):,}" if x >= 1 else f"{x:.0f}"))

ax.grid(True, axis="both", which="major", color="0.85", linewidth=0.6)
