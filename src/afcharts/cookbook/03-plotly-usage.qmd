...
sequential = get_af_colours("sequential", number_of_colours=6)[::-1]
...
In England the rate of new certifications of visual impairment (CVI) due to diabetic eye disease in persons aged 12 years and over, were 3.0 per 100,000 population.
...
sequential = get_af_colours("sequential", number_of_colours=5)[::-1]

# Set default theme
pio.templates.default = "afcharts"

# Source: https://www.gov.uk/government/statistical-data-sets/monthly-domestic-energy-price-statistics
DATA_URL = "https://assets.publishing.service.gov.uk/media/699c5d49cd3d70e6c2f1274c/table_211_213__6_.xlsx"
df = pd.read_excel(DATA_URL, sheet_name="2.1.1", header=11)

df["Year and Period"] = df[["Year and dataset code row", "Quarter"]].astype(str).agg(", ".join, axis=1)

df = df.set_index(["Year and Period"])

# Drop row 0 (contains repeated header artefacts), select numeric block
df = df.iloc[1:, 9:-2].copy()
...
# Wrap each x‑axis label to max 30 characters per line
wrapped_x = ["<br>".join(textwrap.wrap(col.replace("Real terms price indices:", ""), width=30)) for col in df.columns]

fig.add_trace(go.Heatmap(z=df, x=wrapped_x, y=list(df.index), colorscale=sequential))
...
Figures include VAT where applicable.  VAT rates for coal and coke, gas, electricity and heating oils was 8% from the 2nd quarter of 1994 and 5% since the 4th quarter of 1997 (the rate changed on 1st September). 
Real price series are deflated using GDP (market prices) deflator. The GDP deflator is updated when published by ONS. 
Data may change regularly due to rebasing. The ONS use the latest full year of data as the base year and the Department rebases to 2010=100

Note 1. Aggregate of individual solid fuels, gas, electricity and liquid fuels indices.
Note 2. ULSP, ULSD & motor oil. Other fuels are as defined by ONS. See the Methodology tab for further details.
Note 3. ONS update the GDP Deflator on a regular basis.  As such the real terms price indices are subject to regular revisions to the whole series.
</div>
