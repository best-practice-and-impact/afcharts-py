...
By default, `get_af_colours()` will return all colours in the given palette. For the categorical and sequential palettes, the number of colours returned can be set with the `number_of_colours` argument.
...
### Sequential palette
The `sequential` colour palette should be used for data where the order has some meaning, such as age groups. The pale grey colour should be used for cases when, for example, you have no data or have suppressed the value.
